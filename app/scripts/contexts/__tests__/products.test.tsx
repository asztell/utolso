...
describe("<ProductsProvider />", () => {
    ...
    describe("hook", () => {
        ...
        it("throws if hook used outside a provider", () => {
            ...
            expect(() => useProducts()).toThrow("useProducts must be used within ProductsContext")
        });
    });
    ...
    describe("provider", () => {
        ...
        const setup = () => {
            ...
            const { baseElement } = render(<ProductsProvider>
                <ProductsContext.Consumer>{consumerRenderProp}</ProductsContext.Consumer>
        ...
        it("provides the products state", async () => {
            const { result: { products, showProducts } } = setup();
        ...
        it("provides means to dispatch an update products action", () => {
            ...
            const { result: { updateProducts }} = setup();

            updateProducts(newProducts);

            expect(dispatchMock).toHaveBeenCalledWith({ type: "UPDATE_PRODUCTS", payload: newProducts })
        })

        it("provides means to dispatch an update products action", () => {
            const newShowProducts = !state.showProducts;
            const { result: { updateShowProducts } } = setup();

            updateShowProducts(newShowProducts);

            expect(dispatchMock).toHaveBeenCalledWith({ type: "UPDATE_SHOW_PRODUCTS", payload: newShowProducts })
        })
    })
});
